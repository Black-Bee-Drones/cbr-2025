...
MIN_CENTERING_ALTITUDE = 1.0  # Meters, final altitude above figure for landing
TARGET_UP_ALTITUDE = 0.5  # Meters, how high the drone will climb if it doesn't find the package
TARGET_DOWN_ALTITUDE = -0.3  # Meters, distancia de descida para para centralizar
DETECTIONS_LOST_TOLERANCE = 3  # Number of missed detections to consider it as lost
PACKAGE_PROPORTION_ALIGN = 1.43  # How many times the smaller side fits into the larger one to consider it aligned
...
POSITION_CONTROLLER_TOLERANCE_XY = 40  # Meters, XY tolerance for considering the PID at the target
